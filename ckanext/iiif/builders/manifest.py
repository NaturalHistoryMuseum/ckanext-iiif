...
def build_record_manifest(resource: dict, record: dict) -> dict:
    ...
    return {
        ...
        "type": "Manifest",
        "label": _build_label(resource, record),
        "metadata": _build_metadata(record),
        "rights": _build_rights(resource),
        "items": [
            _build_canvas(manifest_id, i, image) for i, image in enumerate(images)
        ],
        "logo": [
            {
...
def _build_record_manifest_id(resource: dict, record: dict) -> str:
    """
    Builds the manifest ID from the given resource and record dicts. If this ID is given
    to the build_iiif_resource action then the manifest for this resource and record
    combination should be returned.
...
def _build_label(resource: dict, record: dict) -> Dict[str, List[str]]:
    """
    Returns the label to use for the given resource and record. Currently, this uses the
    "title field" if specified on the resource. If the "title field" isn't available
    then the record ID is used.
    ...
    if title_field is None:
        title_field = "_id"
    # make sure the value is a string (this should only be necessary if the _id is used)
...
def _build_metadata(record: dict) -> List[Dict[str, Dict[str, list]]]:
    ...
    manifest. This loops through every field in the record and adds it to the returned
    list.
    ...
    for field, value in record.items():
        if isinstance(value, list):
            ...
            value = str(value)
        metadata.append({"label": wrap_language(field), "value": wrap_language(value)})
