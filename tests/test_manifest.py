...
class TestIIIFRecordManifestBuilder(object):
    ...
    def test_label(self):
        builder = IIIFRecordManifestBuilder(
            {'id': 1, '_title_field': 'title'}, {'_id': 1, 'title': 'The title!'}
        )
        assert builder.label == {'en': ['The title!']}

    def test_images_multiple_comma_separated(self):
        builder = IIIFRecordManifestBuilder(
            {'id': 1, '_image_field': 'images', '_image_delimiter': ','},
    ...
    def test_images_single(self):
        builder = IIIFRecordManifestBuilder(
            {'id': 1, '_image_field': 'images'}, {'_id': 1, 'images': 'image1'}
        )
        assert builder.images == ['image1']
    ...
    def test_rights_when_set(self):
        builder = IIIFRecordManifestBuilder(
            {'id': 1, '_image_licence': 'cc-zero'}, {'_id': 1}
        )
